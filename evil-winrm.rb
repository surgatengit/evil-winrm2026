...
require 'logger'
require 'shellwords'
# Readline compatibility for Ruby 3.x (Reline fallback)
begin
  require 'readline'
rescue LoadError
  require 'reline'
...
def completion_check
  if $check_rpath_completion == true
    # Ruby 3.x often uses Reline; don't rely on non-portable Readline APIs
    @completion_enabled = Readline.respond_to?(:completion_proc=)
    unless @completion_enabled
      print_message('Remote path completion is disabled due to ruby limitation: completion API not available', TYPE_WARNING)
      ...
      $conn.shell(:powershell) do |shell|
        begin
          ...
          if Readline.respond_to?(:completion_proc=)
            ...
            Readline.completion_case_fold = true if Readline.respond_to?(:completion_case_fold=)
            Readline.completer_quote_characters = '"' if Readline.respond_to?(:completer_quote_characters=)
          end
            ...
            begin
              if Readline.respond_to?(:bind_key)
                Readline.bind_key("\C-l") do
                  clear_screen
                  # Reline doesn't always support refresh_line; redisplay is the safe option
                  if defined?(Reline) && Reline.respond_to?(:redisplay)
                    Reline.redisplay
                  end
                  nil
              end
